...
注意：部分项目的 record.js 读分镜 meta 里的 `baseUrl`（演示服务地址）——漏写该字段时 `goto('undefined')` 静默失败被 catch 吞掉，后续所有 waitFor 超时而服务本身健康，极易误判成服务问题。record.js 启动时会打印分镜来源与镜 id 清单，录制前务必核对输出的镜标题与分镜表一致（env 没生效/路径写错时会出现「跑的是旧分镜还显示完成」的事故）。
...
- **每镜动作序列第一个动作必须是 `mode`**：每镜独立 context，默认停在首页模式，且 prep 自动化（跳过引导浮层/展开侧栏）挂在 mode 动作上——漏写 mode 时 prep 不跑，引导浮层会盖满整镜画面。**`type/fill` 只检查可见性不做遮挡检查**：浮层盖屏时照样「填写成功」，整镜录完都是浮层画面且不报任何错（实测：只有 `click` 会因遮挡超时报错）。因此每镜尾帧 vision 验证是必选项；可在 record.js 镜结束时检测全屏浮层并打印告警
- **录前选择器预检（强烈推荐）**：分镜写完后、正式录制前，用临时 playwright 脚本打开页面，按各镜 mode 切换后逐个 `waitFor` 验证 actions 引用的选择器真实存在——DOM id 凭记忆写错（实测 `#staffGenNeed` 写成 `#staffDesc`）会让录制中途失败浪费整轮等待。预检只花几十秒，远比失败重录便宜
...
- **抽帧必须用精确 seek**：`-ss <时刻>` 放在 `-i` 之前是关键帧 seek，会偏差数秒，抽到相邻空档造成「字幕丢了」的误判（实测连续三次误判）；放在 `-i` 之后才是精确 seek
- **按 cue 时间轴正中抽帧**：先 `grep "^Dialogue" final.ass` 拿到每条字幕/片尾卡的起止时间，抽帧点选 cue 正中（起止各让 0.5s），不要凭感觉估时间——片尾卡时段就是 ass 里 `EndCard` 样式 Dialogue 的起止（如 `0:02:30,0:02:39`），直接按它抽
