...
# 本地适配: 原版从 autoskill store 读需求统计; 本工具由调用方直接注入 requirement_texts
...
class EvalCompiler:
    def __init__(self, *, config: SkillEvoConfig, sdk: Any = None,
                 requirement_texts: Optional[List[str]] = None) -> None:
        self.config = config
        self.sdk = sdk
        self.requirement_texts = [str(x).strip() for x in (requirement_texts or []) if str(x).strip()]
    ...
    def compile(
        ...
        *,
        ...
        req_texts = list(self.requirement_texts)
        if req_texts:
            corpus = corpus + "\n" + "\n".join(req_texts)
    ...
    def _load_requirement_texts(self, *, user_id: str, offline_lineage_key: str) -> List[str]:
        """本地适配: 已改为构造时注入 requirement_texts, 保留空实现以兼容旧签名."""
        return list(self.requirement_texts)
